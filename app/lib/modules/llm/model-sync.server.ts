...
export class OpenAIExtractor implements ModelExtractor {
  ...
  async extractModels(env: Record<string, string>): Promise<VerifiedModel[]> {
    ...
    const deprecationMap = new Map<
      string,
      { status: 'deprecated' | 'retired'; removalDate?: string; replacement?: string }
    >();
    ...
    try {
      ...
      if (docRes.ok) {
         ...
         */
        const regex =
          /([a-z0-9\-]+(?:-[a-z0-9]+)*).{0,50}?(deprecated|retired|removed|sunset).{0,30}?(\w+\s\d{1,2},\s\d{4}|\d{4}-\d{2}-\d{2})/gi;
...
export class OpenRouterExtractor implements ModelExtractor {
  ...
  async extractModels(_env: Record<string, string>): Promise<VerifiedModel[]> {
    const verifiedModels: VerifiedModel[] = [];
    const fetchedAt = new Date().toISOString();
...
export class ModelSyncService {
  private static _instance: ModelSyncService;
  private _verifiedModels: Map<string, VerifiedModel> = new Map();
  private _extractors: ModelExtractor[] = [
    new OpenAIExtractor(),
  ...
  static getInstance(): ModelSyncService {
    if (!ModelSyncService._instance) {
      ModelSyncService._instance = new ModelSyncService();
    }

    return ModelSyncService._instance;
  }
  ...
  async loadVerifiedModels(): Promise<Map<string, VerifiedModel>> {
    try {
      ...
      this._verifiedModels = new Map(models.map((m) => [`${m.provider}-${m.name}`, m]));

      return this._verifiedModels;
    } catch (e: any) {
      if (e.code === 'ENOENT') {
  ...
  async saveVerifiedModels(models: VerifiedModel[]) {
    try {
      ...
      this._verifiedModels = new Map(models.map((m: VerifiedModel) => [`${m.provider}-${m.name}`, m]));
  ...
  getVerifiedModels(): VerifiedModel[] {
    return Array.from(this._verifiedModels.values());
  }

  getVerifiedModelsForProvider(providerName: string): VerifiedModel[] {
    return Array.from(this._verifiedModels.values()).filter((m: VerifiedModel) => m.provider === providerName);
  }
